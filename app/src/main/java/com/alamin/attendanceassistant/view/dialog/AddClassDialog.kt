...
class AddClassDialog : DialogFragment() {
    ...
    override fun onCreateView(
        ...
        binding.classViewModel = homeViewModel
        binding.lifecycleOwner = this

        if (arg.classModel != null){
            arg.classModel?.let { it -> homeViewModel.setClass(it)  }
        }

        binding.setOnClassSubmit {
            if (arg.classModel == null){
                homeViewModel.createClass()
            }else{
                arg.classModel?.let { it -> homeViewModel.updateClass(it)  }
